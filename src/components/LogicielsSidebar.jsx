...
import {
  IconLayoutDashboard, IconBooks, IconUsers, IconBriefcase,
  IconTool, IconFileText, IconCalculator, IconMenu2, IconSchool,
  IconCalendar, IconMessage,
...
const SectionLabel = ({ text, collapsed }) => {
  ...
  return (
    <div style={{
      fontSize: 10, fontWeight: 600, color: "#7d8590",
...
export default function LogicielsSidebar() {
  ...
  return (
    <div style={{
      ...
      {/* Toggle */}
      <div onClick={() => setCollapsed(c => !c)} title={collapsed ? "Ouvrir" : "Réduire"}
        style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 14, borderBottom: "1px solid #21262d", cursor: "pointer", color: "#7d8590", flexShrink: 0 }}>
        <IconMenu2 size={20} />
      </div>

      <SectionLabel text="Plateforme CPI" collapsed={collapsed} />
      <NavLink href="/dashboard"            icon={IconLayoutDashboard} label="Tableau de bord"  collapsed={collapsed} />
      ...
      <NavLink href="/logiciels" icon={IconSchool} label="Tutoriels" collapsed={collapsed} />

      <SectionLabel text="Ressources" collapsed={collapsed} />
      <NavLink href="/ressources"             icon={IconBooks}    label="Bibliothèque"       collapsed={collapsed} />
      <NavLink href="/ressources?type=TD"     icon={IconFileText} label="Exercices"          collapsed={collapsed} />
      <NavLink href="/ressources?type=projet" icon={IconTool}     label="Projets industriels" collapsed={collapsed} />

      <SectionLabel text="Outils" collapsed={collapsed} />
      <NavLink href="/outils" icon={IconCalculator} label="Calculatrices" collapsed={collapsed} />
